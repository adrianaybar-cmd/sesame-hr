...
const CURRENT_USER_ID = 'emp-6'
const CURRENT_USER_NAME = 'Sofía Romero'
...
function BoostCardItem({ card, onReact }: BoostCardItemProps) {
  ...
  return (
    <Card className={cn('overflow-hidden', card.recipient_id === CURRENT_USER_ID && 'border-yellow-200 bg-yellow-50/30')}>
      <CardContent className="p-4">
        {card.recipient_id === CURRENT_USER_ID && (
          <div className="flex items-center gap-1.5 mb-3">
            <StarIcon className="size-3.5 text-yellow-500" />
            <span className="label-xs font-medium text-yellow-700">Este boost es para ti</span>
          </div>
        )}
        {/* Header */}
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="flex flex-col gap-4 py-2">
          ...
          <div>
            ...
            <Textarea
              placeholder="Escribe un mensaje sincero y específico..."
              value={message}
...
export default function EmployeeBoostPage() {
          ...
          <div className="flex flex-col gap-4 max-w-2xl">
            {myReceived.length === 0 ? (
              <div className="text-center py-12">
                ...
                <span className="paragraph-sm text-muted-foreground">¡Sigue así!</span>
              </div>
            ) : (
              myReceived.map((card) => (
                <BoostCardItem key={card.id} card={card} onReact={handleReact} />
